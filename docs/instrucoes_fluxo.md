...
## 1. Portão da Merrion Road e pré-triagem (P0, P1)

A pré-triagem acontece **no portão e no corredor da lateral leste**, não dentro do Ring 3 (o desenho vigente do Ring 3 não tem garganta).
- O que dizer: "Eleições brasileiras, entrada por aqui. Sabe o número da sua seção? Então procure na tabela: seção → mesa → a letra da fila."
- Quem não sabe a seção vai ao balcão "não sei minha seção" (P1), fora da corrente; não se resolve isso na fila.
- Eleitor do interior (Cork, Galway, Limerick…) procura o nome do condado no índice, não o número da seção.
- Prioritários (~296 no dia, ~33 por hora no pico): o agente de acessibilidade conduz pelo apron pavimentado direto à porta da sua zona. Nenhum entra no serpenteado.

## 2. Corredor da lateral leste e chegada ao Ring 3 (P2)

- A consulta seção → mesa → fila está repetida a cada 25–30 m no corredor. O eleitor decide **andando**; ninguém para na porta.
- O eleitor entra no Ring 3 pelo **canto nordeste**, desce rente ao gradil leste (trecho lateral de 3,0 m) e vira no fundo (trecho de fundo de 3,0 m): é o corredor em L. Do fundo, entra na sua zona.
- **Cruzamento com a saída S8:** a corrente que desce pelo apron leste divide o espaço com quem já votou e sai por S8. Um marshal fica no cruzamento e mantém os dois fluxos separados; quem sai vai para a rua, nunca para o Ring 3.

## 3. Ring 3: uma zona por entrada

Desenho vigente: **Serpenteados horizontais (raias leste-oeste) · CCB só na ponta** — lotação 1.964 pessoas, 82 separadores, 576 m de fita grossa.
...
- Meias-voltas: o marshal mantém o vão livre (1,20 m); guarda-chuvas fechados nas raias.
- **Gatilho:** zona com mais gente do que cabe (A 626, B 712, C 626) → avisar a coordenação; a resposta é abrir ritmo na porta, nunca fechar a entrada da zona.
- Evacuação: as barreiras laterais são removíveis; sai-se pelos vãos entre zonas e pelo gradil. O marshal da zona abre, não conduz.

## 4. Portas de entrada (S4, S5, S6)

- Um marshal por porta. A porta libera enquanto houver lugar nas filas de mesa do lado de dentro (é o que o simulador chama de liberação por buffer).
- Ritmo de pico por porta: A 12,1/min · B 14,1/min · C 12,1/min. Acima disso, avisar a coordenação antes de segurar.
...
- Quem votou sai pela porta de saída do flanco mais próximo. Ninguém volta pelo salão para conversar, fotografar ou esperar alguém.
- Do lado de fora, seguir a placa "SAÍDA / WAY OUT → Merrion Road"; não reentrar no Ring 3 nem cruzar a fila de quem entra.
- S8 despeja no apron leste, onde a corrente de chegada desce: o marshal do cruzamento separa os dois fluxos.

## 8. Acessibilidade e prioridade

- Cerca de 211 eleitores com 60 anos ou mais e 85 com deficiência declarada: ~296 no dia, ~33 por hora no pico.
- Ninguém com prioridade entra no serpenteado (130 m de percurso). Rota: apron pavimentado, do desembarque à porta da sua zona, com o agente de acessibilidade.
- Dentro do salão, o prioritário vai direto à mesa; o orientador de piso avisa o mesário.

## 9. Seguranças e polícia

- 4 seguranças contratados, em postos fixos de controle de acesso e resposta a incidente. Proposta de distribuição: 2 no portão e no acesso, 1 na fachada das entradas, 1 nas saídas. **Não organizam fila**: isso é dos voluntários.
- Polícia do lado de fora do recinto (formato a confirmar com a Garda e registrar na nota verbal).
- Incidente (mal súbito, conflito, objeto suspeito): o voluntário chama o segurança mais próximo e a coordenação; não intervém.
...
- Zona B com mais de 712 pessoas, ou porta S5 recebendo mais de 14,1 por minuto por mais de 10 min.
- Zona C com mais de 626 pessoas, ou porta S6 recebendo mais de 12,1 por minuto por mais de 10 min.
- Fila de mesa além da fita (10 m nas vermelhas, 4 m nos pares) ou fila de mesa sem guia com mais de 8 pessoas.
- Última mesa prevista para fechar depois das 17h03 (dia ruim do simulador).
- Qualquer cruzamento entre quem sai e quem entra fora do apron leste.

## 11. O que muda neste documento se uma decisão mudar

Gerado do registro de decisões: para as duas decisões em aberto e para as decididas que ainda têm alternativas registradas, o que cada alternativa faria com a equipe e com a sinalização interna.

**D1 — Portas: quantas e quais** (decidida em 13/09; hoje: 3 entradas (S4 A, S5 B, S6 C) e 2 saídas (S2, S8))
- se *2 portas (por exemplo S4 e S6)*: sem efeito direto na equipe; ver `docs/decisoes_em_aberto.md`.
- se *4 portas (S3 a S6 ou S4 a S7)*: sem efeito direto na equipe; ver `docs/decisoes_em_aberto.md`.

**D8 — Identidade das filas: letra** (decidida em 13/09; hoje: Letra (A, B, C), como nos rótulos de planejamento)
- se *Cor (azul, âmbar, magenta)*: sem efeito direto na equipe; ver `docs/decisoes_em_aberto.md`.

**D9 — (a) Como fazer a identificação no caderno físico** (em aberto; hoje: sem opção vigente)
- se *(i) Caderno único em ordem alfabética, um mesário identificando*: Nada muda na equipe de piso; o esforço vai para a mesa (mesários).
- se *(ii) Caderno dividido por seção agregada, um mesário por caderno*: Pré-triagem precisa dizer ao eleitor a SEÇÃO, não só a mesa: o painel seção → mesa ganha a coluna do caderno.
- se *(iii) Caderno dividido por faixa de letras, um mesário por faixa*: Placa da mesa com as faixas de letras (A–L / M–Z); orientador aponta a fila certa.
...
- se *Raias norte-sul, barreira inteira (1.997)*: sem efeito direto na equipe; ver `docs/decisoes_em_aberto.md`.
