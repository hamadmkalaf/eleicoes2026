...
function simularDia(mont, seed){
  ...
  let perdidos = 0;                                // sem checkpoint: quantos erraram o caminho
  ...
  function tentaLiberar(z, t){
    ...
    while (areaFila[z].length) {
      ...
      if (comCp) {
        ...
        // Sem checkpoint: 10 s para ler a sinalização na porta e a caminhada
        // direta. Se `sinalizacao.erro` > 0, essa fração dos eleitores se
        // perde e caminha `sinalizacao.desvio` m a mais antes de achar a mesa.
        const sin = cen.sinalizacao || {};
        const desvio = (sin.erro > 0 && rnd() < sin.erro) ? (sin.desvio || 0) : 0;
        if (desvio) perdidos++;
        est[v.mesa].transito++;
        heap.push(t + 10 + (porSlot[v.mesa].distCp + desvio) / VEL, EV.MESA_CHEGADA, v);
      }
    }
  }
  ...
  return {
    ...
    saidasPorta, liberadosPorta, perdidos,
  };
}
